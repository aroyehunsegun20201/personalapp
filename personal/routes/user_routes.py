...
from  personal import app,csrf,myforms,db,models, Message
from personal import mail


#### sending authomatic mail 
#### to use and confirm - myaccount.google/lesssecureapp
@app.route('/user/test')
def testmail():
    msg=Message(subject="Testing mail", sender="aroyehunsegun20201@gmail.com", recipients=['aroyehunsegun@yahoo.com'])
    fp = open('/personal/requirement.txt')
    msg.html = "<div> <h1> welcome aboard </h1> <p>our best customer</p><hr> signed by management<img src""</div>"
    mail.attach("requirements.txt", "application/text", fp.read())
    mail.send(msg)
    return" mail was sent"
    # Message()   #if issues arise remove Message()
    # msg=Message(subject="Testing mail", sender="aroyehunsegun20201@gmail.com", recipients=['aroyehunsegun@yahoo.com'], body="test mail")
    # check= mail.send(msg)
    # if check:
    #     return 'Mail was sent'
    # else: 
    #     return 'checking mail semding failed...'
...
def confirmpay():
    if session.get('user') !=None and session.get('trxref') !=None:
        ...
        if request.method=='GET':
            ...
            headers = {"Content-Type": "application/json","Authorization":"Bearer sk_test_here45678905"}            
